# Spec: Memory Ingest — Batch Mode (`--all`)

> Merged from `openspec/changes/memory-ingest-batch` on 2026-04-08.
...
- `--change` defaults to `''` (empty string, falsy) when `--all` is used; the mutual-exclusivity guard relies on truthiness so no false positives occur.
- Empty-candidates check fires when zero non-archive directories exist, before any proposal check.
- `archive/` is excluded by directory name comparison, not by reading `.openspec.yaml` status.
